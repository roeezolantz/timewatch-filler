...
class TimeWatch:
    def __init__(self, loglevel=logging.WARNING, **kwargs):
        ...
        self.loginpath = "user/validate_user.php"
        # https://c.timewatch.co.il/user/validate_user.php
    ...
    def login(self, company, user, password):
        ...
        user - id/employee number is filled by user when logging in
        password - password is filled by user when logging in
        ...
        if not ("שלום" and "התנתק") in r.text:
        ...
        self.logger.info(f"successfully logged in as {self.user} with id {self.employeeid}")
    ...
    def time_to_tuple(self, t):
        ...
        if not isinstance(t, tuple) or not len(t) == 2:
            raise Exception(f"couldn't convert time to tuple: {t}")
    ...
    def edit_date(self, year, month, date, date_duration):
        ...
        if isinstance(current_date_duration_hour, str):
            # if there is no punch at att, set the var to 100
            current_date_duration_hour = 100
            current_date_duration_min = 100

        what_to_punch = None
        # Scenario 1: The duration is above 9+ hours, no need to update
        if 9 <= current_date_duration_hour < 100 and 1 <= \
                current_date_duration_min <= 59:

            return None  # No need to punch, already beyond 9.x hours

        # Scenario 2: The duration is below 9 hours, and only one punch at the BEGINNING of the
        # day, we need to update2.
        ...
        # Scenario 4: The duration is below 9 hours, but two punches were made.
                ...
                and date_duration[date][0][0] < 9:
            if date_duration[date][1][0] > 9:
                # Calculating how many hours we need
                delta1 = datetime.timedelta(hours=9, minutes=5 + jitter)
                delta2 = datetime.timedelta(hours=date_duration[date][0][0],
                                            minutes=date_duration[date][0][1])
        ...
        while not (
            ...
            if failures >= self.retries:
                self.logger.warning(f'Failed punching in {failures} times on {date}')
                return False
        return True
    ...
    def edit_date_post(self, date, year, month, start=None, end=None,
        ...
        self.logger.info(
            f'Punched in on {date_str} as {self.tuple_to_str(start)} to {self.tuple_to_str(end)}')
        return True
    ...
    def parse_dates(self, year, month, keep_cause):
        """Which dates to work on, it gets the weekdays which has working day or Thursday in the
        right tab. Then, process them into the next function.
        punchclock4.jpg and internet4.jpg = real punch with the tag or from the internet,
        cannot be changed! oved4.png - May be changed.
        The output will be as follows:
        {current duration}, {first entry}, {what punched it? ( or none )} , {second entry},
        {what punched it? ( or none )}, ..., same for 3 and 4... """
                ...
                for i in range(4, 8):  # Get entries and punches
                    ...
                    if len(tds[i].contents[0].contents) == 2:  # Check if it's not empty. reducing faults.
                        if tds[i].contents[0].contents[1] is not None:
                            if tds[i].contents[0].contents[1].attrs['src'] == \
                                    '/images/punchclock4.svg':
                                date_durations[date].append('punched_no_change')
                            elif tds[i].contents[0].contents[1].attrs['src'] == '/images/oved4.svg':
                                date_durations[date].append('hand')
                            elif tds[i].contents[0].contents[1].attrs['src'] == \
                                    '/images/internet4.svg':
                                date_durations[date].append('punched_no_change')
                        ...
                        self.logger.debug(
                            f'parsed expected duration for {date} as {date_durations}')
    ...
    def validate_date(self, year, month, expected_date, expected_start, expected_end, what_to_punch):
        ...
        for tr in beautifulsoup(r.text).findAll('tr', attrs={'class': 'update-data'}):
            ...
            if start != expected_start or end != expected_end:
                self.logger.info(
                    f'Validation failed, expected: {expected_start} - {expected_end}. read: '
                    f'{start} - {end}')
                return False

            self.logger.debug(f'Successful validation for {expected_date} as {start}-{end}')
            return True

        self.logger.info(f'Validation failed reading punch time for {expected_date}')
        return False
